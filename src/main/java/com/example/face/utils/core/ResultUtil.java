package com.example.face.utils.core;
...
//		return new Result().setCode(ResultCode.FAIL).setMessage(message);
		...
		Head head = new Head(ResultCode.UNAUTHORIZED, message, FALSE);
		return new Result().setCode(ResultCode.UNAUTHORIZED).setMessage(message).setHead(head);
	}
